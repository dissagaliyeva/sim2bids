...
class NoCodeModel:
    ...
    def save_params(self, k, v):
        ...
        xml = os.path.join(f'../sim2bids/models/models/{self.model_name}.xml')
        ...
        if not os.path.exists(os.path.join(app.OUTPUT, 'eq')):
            os.mkdir(os.path.join(app.OUTPUT, 'eq'))
...
class XML:
    ...
    def check_params(self):
        if 'hindmarsh' in self.model_name.lower():
            comp_name = 'SJHM3D'
        elif 'wong_wang' in self.model.lower():
            comp_name = 'RWW'
        else:
            comp_name = 'G2DOS'
...
class CodeModel:
    ...
    **params:           dict
    ...
    def create_params(self):
        ...
        # define model's type
        if self.model_name in ['hindmarshrose', 'hindmarsh', 'hindmarsh_rose']:
    ...
    def save_xml(self, model, ftype='default'):
        ...
        if ftype == 'default':
            if self.suffix:
                self.path = os.path.join(app.OUTPUT, 'param', f'desc-{self.suffix}_param.xml')
            else:
                if self.uid != 'default':
                    self.uid = self.uid.split('_')[0]
                self.path = os.path.join(app.OUTPUT, 'param', f'{self.uid}_param.xml')

            # model.export_to_file(self.path)
            self.model = model
        elif ftype == 'model':
            print('Model to be saved:', self.model)
            self.merge_xml()
    ...
    def merge_xml(self):
        ...
        """
        self.path = f'{app.OUTPUT}/eq/desc-{app.DESC}_'
        ...
        if os.path.exists(self.path):
            # xml1 = self.path
            xml2 = f'../sim2bids/generate/models/{model_name}.xml'
            file = os.path.join(app.OUTPUT, 'eq', f'model-{self.comp_type}_{self.uid}.xml')
            ...
            shutil.copy(xml2, path)
            convert.to_json(path.replace('xml', 'json'), shape=None,
                            desc=utils.TVB_MODELS[self.comp_type]['desc'], key='eq')
#
# def set_params(conversion_name='default', rhythm=None, **kwargs):
#     # set the default name for the conversion
