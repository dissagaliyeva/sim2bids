...
get_ipython().system('pip install -U tvb-library==2.6 --quiet')
get_ipython().system('pip install sim2bids==0.1.1 --quiet')
...
white_matter_coupling = coupling.Linear(a=np.array([0.0154]))
...
what_to_watch = (mon_raw, mon_tavg, mama)
...
# # Falculate functional connectivity
...
# save time series (bold)
pd.DataFrame(BOLD[:, 0, :, 0], index=None).to_csv(os.path.join(path, 'ts_bold.txt'), sep='\t', header=None, index=None)

# save times for bold ts 
pd.DataFrame(bold_time, index=None).to_csv(os.path.join(path, 'bold_times.txt'), sep='\t', header=None, index=None)

# save fc 
pd.DataFrame(fc, index=None).to_csv(os.path.join(path, 'fc.txt'), sep='\t', header=None, index=None)
...
get_ipython().system('jupyter nbconvert tutorial_s1_region_simulation_bids_1509.ipynb --to python')
...
# zip inputs
get_ipython().system('zip -r inputs.zip connectivity_76')
