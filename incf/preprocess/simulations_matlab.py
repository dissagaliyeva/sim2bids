...
def save(subs, path, ses=None):
    ...
    try:
        ...
        convert_mat(mat, subs, path, ses=ses)


def save_mat73(subs, path, ses=None):
    mat = mat73.loadmat(subs['path'])
    convert_mat(mat, subs, path, ses=ses)


def convert_mat(mat, subs, path, ses=None):
    ...
    if len(data) == 0:
        ...
        if ses is None:
            ts_path = os.path.join(path, sid, 'ts')
            ...
            ts_path = os.path.join(path, sid, ses, 'ts')
            ...
            convert.create_sub_struct(path, subs, ses=ses)
